		...
		public String input(int number) {
			...
			String answer = "";	
				...
				if (number < 10) {
					...
					int digit1 = number % 10;  /// Finds last digit from modulus
					int digit2 = (number/10) % 10;	/// Finds second last digit by taking away 
					int digit3 = (number/100) % 10;	/// Finds third last digit by taking away 
					answer = number + " is made up of three digits: " +
							digit3 + " and " + digit2 + " and " + digit1;	
					
					Numbers wordForm = new Numbers();
					String digit3WordSecond = wordForm.numToWordConverter(digit3*100); 
					String digit3WordFirst = wordForm.numToWordConverter(digit3); 
					if (digit2 >= 1 && digit2 < 2) {
						String digit1Word = wordForm.numToWordConverter(number/10); 
						System.out.println(digit3WordFirst + "-" + digit3WordSecond + " and " + digit1Word);
					} else {					
						String digit1Word = wordForm.numToWordConverter(digit1); 
						String digit2Word = wordForm.numToWordConverter(digit2*10); 
						System.out.println(digit3WordFirst + "-" + digit3WordSecond + " and " + digit2Word + "-" + digit1Word);
					}	
